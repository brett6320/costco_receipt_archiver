...
def _num(v):
    """Coerce orderTotal, which may be a number or {'parsedValue': n}."""
    if isinstance(v, dict):
        v = v.get("parsedValue", v.get("source", 0))
    try:
...
def find_online_orders(resp: Any) -> list[dict]:
    """Extract getOnlineOrders bcOrders[] from a response, into receipt schema.

    Online line items carry itemNumber/itemDescription but NO per-item price;
    only the order total is available. For single-line orders we put the total
    on that line; multi-line orders keep line amounts at 0 with the total on the
    receipt header (subTotal/total).
    """
    bc_orders = []

    def walk(node):
        if isinstance(node, dict):
            if "bcOrders" in node and isinstance(node["bcOrders"], list):
                bc_orders.extend(node["bcOrders"])
            for v in node.values():
                walk(v)
        elif isinstance(node, list):
            for v in node:
    ...
    receipts = []
    for o in bc_orders:
        date = str(o.get("orderPlacedDate") or o.get("orderedDate") or "")
        num = str(o.get("orderNumber") or o.get("sourceOrderNumber")
                  or o.get("orderHeaderId") or "")
        total = _num(o.get("orderTotal"))
        lines = o.get("orderLineItems") or []
        single = total if len(lines) == 1 else 0.0
        items = []
        for li in lines:
            items.append({
                "itemNumber": str(li.get("itemNumber") or li.get("itemId") or "").strip(),
                "itemDescription01": li.get("itemDescription") or "",
                "itemDescription02": "",
                "unit": 1,
                "amount": single,
                "itemUnitPriceAmount": single,
                "taxFlag": "",
            })
        receipts.append({
            "transactionBarcode": num,
            "transactionDateTime": date,
            "transactionDate": date[:10],
            "warehouseName": "Online Order",
            "warehouseNumber": str(o.get("warehouseNumber") or ""),
            "documentType": "online",
            "transactionType": o.get("status") or "Online",
            "total": total,
            "subTotal": total,
            "taxes": 0.0,
