...
def _load_or_login(force_login: bool = False, timeout: int = 300) -> Credentials:
    ...
    config.ensure_dirs()
    return login_and_get_credentials(timeout_seconds=timeout, cred_cache=CRED_CACHE)


def cmd_login(args) -> None:
    creds = login_and_get_credentials(
        timeout_seconds=args.timeout, cred_cache=CRED_CACHE
    )
...
def cmd_fetch(args) -> None:
    ...
    creds = _load_or_login(timeout=args.timeout)
...
def cmd_online(args) -> None:
    ...
    _load_or_login(timeout=args.timeout)
